...
extract_date <- function(filepath) {
  ...
  date <- substr(basename, regexpr("_", basename) + 1, regexpr("_", basename) + 8)
  # print(date)
...
filter_by_date_range <- function(strings, date_range) {
  ...
  date_range_converted <- unlist(strsplit(date_range, ",\\s*"))

  start_date <- as.Date(date_range_converted[1])
  end_date <- as.Date(date_range_converted[2])
  
  print(paste("startdate:", start_date))
  print(paste("enddate:", end_date))
  ...
  # print(paste("fitlered strings:", filtered_strings))
...
build_virtual_raster_stack <- function(gdal_path, list, list_source_dir, output_dir) {
  ...
  if (Sys.info()["sysname"] == "Windows") {
    # Build the full path to gdalbuildvrt on Windows
    buildvirtpath <- paste(gdal_path, "gdalbuildvrt.exe", sep = "\\")
  } else {
    # Use the command directly on Linux
    buildvirtpath <- paste(gdal_path, "gdalbuildvrt", sep = "/")
  }
  ...
  path_to_vrt_output_file <- file.path(
    output_dir, paste0(basename(list_source_dir), "_stack.vrt")
  )
  
  # Fill the text file with the paths of the found layers 
  # from which the VRT file  should be created
  ...
  buildvrt_params <- c(
    "-separate",
    "-input_file_list", 
    shQuote(path_to_input_list_txt_file),
    "-overwrite",
...
createMonthlyComposites_ndvi_lst <- function(gdal_path, s3_preprocessed_dir, date_range, output_dir){
  
  # Define path to LST and NDVI folders
  lst_dir <- file.path(s3_preprocessed_dir, "LST")
  ndvi_dir <- file.path(s3_preprocessed_dir, "NDVI")
  ...
  # print(paste("lst list:", lst_list))
  # print(paste("ndvi list:", ndvi_list))
  ...
  if (length(lst_list) > 0 && length(ndvi_list) > 0){
    ...
    for (i in 1:length(divided_lst_lists)) {
      ...
      build_virtual_raster_stack(
        gdal_path, 
        unlist(divided_lst_lists[[i]], recursive = F), 
        lst_dir, 
        output_dir
      )
      build_virtual_raster_stack(
        gdal_path, 
        unlist(divided_ndvi_lists[[i]], recursive = F), 
        ndvi_dir, 
        output_dir
      )
      ...
      print(
        paste(
          "LST_stack.vrt created! It contains", 
          length(divided_lst_lists[[i]]), 
          "layer.", 
          sep = " "
        )
      )
      ...
      print(
        paste(
          "NDVI_stack.vrt created! It contains", 
          length(divided_ndvi_lists[[i]]), 
          "layer.", 
          sep = " "
        )
      )
      ...
      path_to_lst_composite_output_file <- file.path(
        output_dir, 
        paste0(
          "LST_composite_", names(divided_lst_lists)[i], ".tif"
        )
      )
      path_to_ndvi_composite_output_file <- file.path(
        output_dir, 
        paste0(
          "NDVI_composite_", names(divided_ndvi_lists)[i], ".tif"
        )
      )
      
      # Write the composite rasters to disk
      terra::writeRaster(
        max_lst_composite, path_to_lst_composite_output_file, 
        filetype = "GTiff", overwrite = TRUE
      )
      terra::writeRaster(
        max_ndvi_composite, path_to_ndvi_composite_output_file, 
        filetype = "GTiff", overwrite = TRUE
      )
    ...
    print(
      "No matching LST and NDVI tifs were found for the specified 
      period. Therefore, no composites can be created."
    )
  }
}
...
createMonthlyComposites_ndii <- function(gdal_path, s3_preprocessed_dir, date_range, output_dir){

  # Define path to NDII folder
  ndii_dir <- file.path(s3_preprocessed_dir, "NDII")
  
  # Get .tif files in NDII folder
  ndii_list <- list.files(ndii_dir, pattern = "*.tif$", full.names = TRUE)
  ndii_list <- filter_by_date_range(ndii_list, date_range)
  # print(paste("ndii list:", ndii_list))
  
  divided_ndii_lists <- divide_strings_by_year_month(ndii_list)
  # print(paste("divided ndii list:", divided_ndii_lists))
  ...
  for (i in 1:length(divided_ndii_lists)) {
    ...
    build_virtual_raster_stack(
      gdal_path, 
      unlist(divided_ndii_lists[[i]], recursive = F), 
      ndii_dir, 
      output_dir
    )
    if (!file.exists(path_to_ndii_vrt_output_file)) {
      print(
        "No matching NDII tifs were found for the specified 
        period. Therefore, no composites can be created."
      )
    ...
    print(
      paste(
        "NDII_stack.vrt created! It contains", 
        length(divided_ndii_lists[[i]]), 
        "layer.", 
        sep = " "
      )
    )
    ...
    path_to_ndii_composite_output_file <- file.path(
      output_dir, 
      paste0("NDII_composite_", names(divided_ndii_lists)[i], ".tif")
    )
    
    terra::writeRaster(
      median_ndii_composite, 
      path_to_ndii_composite_output_file, 
      filetype = "GTiff", overwrite = TRUE
    )
...
calibrateMonthlyComposites_ndvi_lst <- function(composites_dir, calibratedOutput_dir){
  ...
  lst_list <- list.files(
    composites_dir, 
    pattern = "LST_composite_\\d{4}-\\d{2}\\.tif$", 
    full.names = TRUE
  )
  ndvi_list <- list.files(
    composites_dir, 
    pattern = "NDVI_composite_\\d{4}-\\d{2}\\.tif$", 
    full.names = TRUE
  )
  ...
  for (i in 1:length(lst_list)) {
    ...
    # Create file names
    ndvi_file_name <- paste0(
      file_path_sans_ext(basename(ndvi_list[i])), '_calibrated'
    )
    lst_file_name <- paste0(
      file_path_sans_ext(basename(lst_list[i])), '_calibrated'
    )
    ...
    writeRaster(ndvi_calibrated, paste0(ndvi_file_path, '.tif'), overwrite = TRUE)
    print(paste("Successfully created ", ndvi_file_name, ".tif", "!"))
    
    writeRaster(lst_calibrated, paste0(lst_file_path, '.tif'), overwrite = TRUE)
    print(paste("Successfully created ", lst_file_name, ".tif", "!"))
...
calibrateMonthlyComposites_ndii <- function(composites_dir, calibratedOutput_dir){
  ...
  ndii_list <- list.files(
    composites_dir, 
    pattern = "NDII_composite_\\d{4}-\\d{2}\\.tif$", 
    full.names = TRUE
  )
  ...
  for (i in 1:length(ndii_list)) {
    ...
    ndii_file_name <- paste0(file_path_sans_ext(basename(ndii_list[i])), '_calibrated')
    
    output_dir <- calibratedOutput_dir
    ndii_file_path <- paste0(output_dir, "/", ndii_file_name)
    writeRaster(ndii_calibrated, paste0(ndii_file_path, '.tif'), overwrite = TRUE)
    print(paste("Successfully created ", ndii_file_name, ".tif", "!"))
  }
}